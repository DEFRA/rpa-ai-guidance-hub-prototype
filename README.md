...
```bash
cd rpa-ai-guidance-hub-prototype
nvm use
```
...
> [!WARNING]
> The `rpa-ai-guidance-hub-prototype` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `rpa-ai-guidance-hub-prototype`. To turn it
...
```bash
docker build --target development --no-cache --tag rpa-ai-guidance-hub-prototype:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 rpa-ai-guidance-hub-prototype:development
```
...
```bash
docker build --no-cache --tag rpa-ai-guidance-hub-prototype .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 rpa-ai-guidance-hub-prototype
```
...
```bash
docker run -it --entrypoint /bin/ash rpa-ai-guidance-hub-prototype
```
