...
// with these two items for /v5/... (or, via the identical V6_NAVIGATION
// block just below, /v6/...) requests only, leaving v1 and v2 exactly as
// they already were (still the plain border, still driven by the untouched
// top-level `navigation` array). "Current" is worked out by path prefix,
// not exact match like the site-wide version above, since a tab should stay
// highlighted across its whole page cluster — Find guidance for every
// /v5/find-guidance/... and /v5/document-overview/... page (organic search,
// document overview, the document itself, all reached from
// find-guidance.html), Manage guidance for every /v5/all-guidance-docs,
// /v5/guidance-document/... and /v5/manage-guidance/... page (its own search
// results and document overview). Neither tab highlights on
// start/editor-experiment, same as a real top nav showing no active tab on
// pages outside both sections — and the bar does not render at all on
// /v5/sign-in (isV5SignIn skips the override below, leaving
...
// (see the "Version 6" block further down), so this is kept as its own
// separate constant rather than v6 reusing V5_NAVIGATION with its hrefs
// patched at render time, the same reasoning the routes themselves are not
// shared: v5's own nav should not change if this one is edited later.
const V6_NAVIGATION = [
  { text: 'Find guidance', href: '/v6/find-guidance', prefixes: ['/v6/find-guidance', '/v6/document-overview'] },
  { text: 'Manage guidance', href: '/v6/all-guidance-docs', prefixes: ['/v6/all-guidance-docs', '/v6/guidance-document', '/v6/manage-guidance'] }
]
...
router.use((req, res, next) => {
  if (req.path.startsWith('/v1/')) {
    ...
    } else if (req.path === '/v6/find-guidance' || req.path === '/v6/all-guidance-docs') {
